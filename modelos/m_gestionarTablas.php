<?php
  ...
  public function getDatosTabla($id)
    ...
    public function devolverDatosEjercicioTabla($id){
      ...
      return $result;
		...
		public function crearTabla($id, $nombre, $tipe, $ins)
    {

        $bd = conexion_BD();
        $stmt = $bd->prepare("INSERT INTO tabla (idTabla, nombre,tipo,instrucciones) VALUES ( ?, ?, ?, ?)");
        $stmt->bind_param('ssss', $id, $nombre, $tipe, $ins);
  ...
  public function borrarDeportistaAsignadoTablaPEF($id, $dni){
        $bd = conexion_BD();
        $stmt=$bd->query("DELETE FROM deportista_tiene_tabla WHERE Tabla_idTabla ='".$id."' AND  Usuario_DNI ='".$dni."'");
  ...
  public function modificartablaTieneEjercicio($idEjercicio, $id){
      ...
      $bd = conexion_BD();
        ...
        $mensaje = "La tabla ha sido modificada";
                  echo "<script>";
                  echo "alert('$mensaje');";
                  echo "window.location = '../vistas/v_verTabla.php?id=$id'";
                  echo "</script>";
        }
	...
	public function modificarTabla($id,$nombre,$tipo,$ins)
	{
		$bd = conexion_BD();
		$stmt = $bd->query("UPDATE tabla SET nombre='$nombre', tipo='$tipo', instrucciones='$ins'  where idTabla='".$id."' ");
