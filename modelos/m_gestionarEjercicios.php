...
class m_gestionarEjercicios
{
  ...
  public function contarEjercicio($dni,$ejercicios, $tabla)
  {
      ...
      if(count($ejercicios) != 0)  //Si se ha completado algun ejercicio, se tiene en cuenta en el historial
      {
        $fecha = getdate();

        $f = $fecha['year']."-".$fecha['mon']."-".$fecha['mday']." ".$fecha['hours'].":".$fecha['minutes'].":".$fecha['seconds']; //Se guarda en dos formatos para poder ordenarla
      ...
      $mensaje = "Ha terminado la tabla";
			echo "<script>";
			echo "alert('$mensaje');";
			echo "window.location = '../vistas/v_misTablas.php'";
			echo "</script>";
